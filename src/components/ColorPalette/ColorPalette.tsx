...
export const ColorPalette: React.FC = () => {
  const { colorRegisters, setColorRegister } = useAppStore();
  ...
  const currentLuma = getLumaStep(currentVal);
  ...
  return (
    <div className="glass-panel rounded-xl p-4 flex flex-col gap-4 shadow-lg border border-zinc-800">
      ...
      {/* Active Register Palette Picker Grid (16 Hues x 8 Lumas) */}
      <div className="bg-zinc-900/90 p-3 rounded-lg border border-zinc-800 flex flex-col gap-2">
        <div className="flex items-center justify-between text-xs">
          <span className="text-zinc-300 font-medium flex items-center gap-1.5">
            ...
            <strong className="text-sky-400">{ATARI_HUE_NAMES[currentHue]} ({currentHue})</strong>, Luma{' '}
            <strong className="text-emerald-400">${(currentLuma * 2).toString(16).toUpperCase()}</strong>
          </span>
        ...
        <div className="grid grid-cols-16 gap-0.5 p-1.5 bg-black rounded border border-zinc-800 overflow-x-auto">
          {Array.from({ length: 16 }).map((_, hue) => (
            <div key={hue} className="flex flex-col gap-0.5">
              {Array.from({ length: 8 }).map((_, luma) => {
                const atariByte = makeAtariByte(hue, luma);
                const hex = atariByteToHex(atariByte);
                const isCurrent = currentVal === atariByte;

                return (
                  <button
                    key={luma}
                    onClick={() => setColorRegister(activeReg, atariByte)}
                    title={`Hue: ${hue} (${ATARI_HUE_NAMES[hue]}), Luma: ${luma * 2} ($${atariByte.toString(16).toUpperCase().padStart(2, '0')})`}
